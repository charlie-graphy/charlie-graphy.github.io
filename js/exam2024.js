...
function showContent(idx){
	...
	switch(Number(idx)){
		case 0:  //싱클레어
			...
			$('.recommendCont .recCont2 .page-content').append($('<div>',{'class':'card','style':'background-image:url("https://i.ytimg.com/vi/TgmCnvcGI4U/maxresdefault.jpg")'}).append($('<div>',{'class':'content'}).append($('<h4>',{'class':'title','text':'편지'}), $('<p>',{'class':'copy','text':'"테를 늘리는 일이 얼마나 고통스러울지"'}), $('<button>',{'class':'btn','text':'보러가기','onclick':'window.open("https://youtu.be/TgmCnvcGI4U")'}))));
			$('.recommendCont .recCont2 .page-content').append($('<div>',{'class':'card','style':'background-image:url("https://i.ytimg.com/vi_webp/9XxQQXTtQY8/maxresdefault.webp")'}).append($('<div>',{'class':'content'}).append($('<h4>',{'class':'title','text':'수용'}), $('<p>',{'class':'copy','text':'"그건 엄청난 용기가 필요한 거라고"'}), $('<button>',{'class':'btn','text':'보러가기','onclick':'window.open("https://youtu.be/9XxQQXTtQY8")'}))));
			break;
		case 1: //토루
