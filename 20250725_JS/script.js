...
$(document).ready(function () {
	...
	function getPotionResult(song, act, ene) {
		  ...
		  if (song === 100 && act < 100 && ene < 100) {
		    return makeResult('노래 맛집 포션', '목소리 하나로 무대를 꽉 채우는 노래 맛집이에요.', '🎤');
		  }
		  if (act === 100 && song < 100 && ene < 100) {
		    return makeResult('눈빛 장인 포션', '한 번 보면 빠져드는 몰입 연기 장인이에요.', '🎭');
		  }
		  if (ene === 100 && song < 100 && act < 100) {
		    return makeResult('불꽃 폭발 포션', '뜨거운 에너지로 무대를 불태우는 타입이에요.', '⚡️');
		  }
		  if (song + act >= 180) {
		    return makeResult('울림 포션', '한 장면, 한 소절마다 마음을 흔드는 진한 울림이 있어요.', '🎶');
		  }
		  if (song + ene >= 180) {
		    return makeResult('전율 포션', '진심이 담긴 노래와 에너지로 무대에 강렬한 전율을 선사해요.', '💫');
		  }
		  if (act + ene >= 180) {
		    return makeResult('태양 포션', '터질 듯한 에너지와 몰입 연기로 무대를 장악하는 태양이에요.', '☀️');
		  }
		  if (song >= 90 && act >= 90 && ene >= 90) {
		    return makeResult('레전드 포션', '“어떻게 이 모든 걸 다 잘하지?”라는 말이 절로 나오는 올라운더예요.', '🧪');
		  }
		  if (song === 0 && ene >= 90) {
		    return makeResult('존재감 포션', '어떤 장면에서도 단번에 눈을 사로잡는, 무대 위의 중심이에요', '👀');
		  }
		  if (act === 0 && song >= 90) {
		    return makeResult('음유시인 포션', '감정을 담아 노래로 마음을 움직이는 음유시인이에요.', '🎧');
		  }
		  if (ene === 0 && act >= 90) {
		    return makeResult('감정 연기 포션', '진심을 담은 연기로, 담백하지만 깊은 울림을 전해요.', '🎬');
		  }
		  if (song >= 40 && song <= 60 && act >= 40 && act <= 60 && ene >= 40 && ene <= 60) {
		    return makeResult('올라운더 포션', '무대 위 어디서든 빛나는 다재다능한 매력을 가진 존재예요.', '✨');
		  }
		  if (song <= 30 && act <= 30 && ene <= 30) {
		    return makeResult('미지의 포션', '감히 평가할 수 없는 미지의 영역이에요.', '🌀');
		  }
		  if (song > act && ene >= 40 && ene <= 60) {
		    return makeResult('음색 천재 포션', '진심을 담은 목소리로 무대를 매력적으로 물들이는 음색 천재예요.', '🎵');
		  }
		  if (act > ene && song <= 30) {
		    return makeResult('연기 천재 포션', '깊은 몰입과 섬세한 표현으로 무대를 압도하는 연기 천재예요.', '🎥');
		  }
		  if (ene > song && ene > act && avg < 50) {
		    return makeResult('분위기 포션', '빛나는 에너지와 자연스럽게 스며드는 매력으로 무대를 채워요.', '🪞');
		  }
